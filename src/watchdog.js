...
export function planWatchdogActions(state, heartbeats, input = {}) {
  ...
  const operatorPaused = Boolean(state.meta?.operatorPause?.active);
  const budgetResumesAt = Date.parse(state.meta?.budgetPause?.resumesAt || "");
  const budgetPaused = Boolean(
    state.meta?.budgetPause?.active
    && (!Number.isFinite(budgetResumesAt) || budgetResumesAt > nowMs),
  );
  const newWorkPaused = operatorPaused || budgetPaused;
  const queuedRunWaiting = !newWorkPaused && (state.runs || []).some((run) => {
    if (run.status !== "queued" || ageMs(run.createdAt, nowMs) <= workWaitMs) return false;
    const project = (state.projects || []).find((item) => item.id === run.projectId);
    const task = (state.tasks || []).find((item) => item.id === run.taskId);
    return !["open", "probe_required"].includes(project?.automationCircuit?.state)
      && !["open", "probe_required"].includes(task?.automationCircuit?.state);
  });
  ...
  const dispatchWaiting = !newWorkPaused && (state.tasks || []).some((task) => (
    ...
    && !["open", "probe_required"].includes(task.automationCircuit?.state)
    && !["open", "probe_required"].includes(
      (state.projects || []).find((project) => project.id === task.projectId)?.automationCircuit?.state,
    )
  ));
