...
export async function recordReview(taskId, input = {}) {
  return mutateState(async (state) => {
    ...
    const cycleLimitLeadDecision = cycleLimitLeadReviewApplies(state, project, task, stage);
    if (earliestRequiredStage && stageIndex > earliestRequiredIndex && !cycleLimitLeadDecision) {
      throw new Error(
