...
test("promotion merges an approved task branch into the target branch after validation", async () => {
  ...
  try {
    ...
    const state = JSON.parse(await readFile(path.join(root, "data", "mission-control.json"), "utf8"));
    const remoteFeature = await git(root, ["--git-dir", remotePath, "show", "refs/heads/main:feature.txt"]);

    assert.equal(report.projects[0].status, "ready");
    assert.equal(report.projects[0].tasks[0].status, "promoted");
    assert.equal(remoteFeature, "feature");
    assert.equal(state.tasks[0].status, "merged");
    assert.equal(state.tasks[0].promotionStatus, "promoted");
    assert.ok(state.tasks[0].promotionCommit);
    assert.ok(state.events.some((event) => event.type === "release_candidate_ready"));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
