...
- Project repositories and validation commands are owner-authorized executable inputs. QA and promotion validate exact commits in disposable full clones under a macOS Seatbelt policy with a synthetic credential-free environment, kernel-enforced filesystem and executable allowlists, denied network access, and a narrow non-process system-information allowlist. A pre/post byte, mode, and symlink manifest detects persistent checkout changes; it does not prevent code from changing and restoring files during a command, so it is not a hostile semantic-validation boundary. Timeout and output limits bound the leader, ordinary process group, and inherited output pipes; macOS does not guarantee termination of a descendant that deliberately creates a new session or process group, so this is not a hostile-code or host-availability boundary.
- A merged promotion becomes durable only after two independent observations: an exact, redirect-disabled GitHub API result and a fresh protected-branch reachability check in a new private bare repository. The ancestry verifier fetches the canonical HTTPS repository directly with local/global Git configuration and non-HTTPS transports disabled, binds the result to the current promotion-claim lease generation, and destroys its temporary repository before returning an uncloneable in-process seal. The SQLite promotion writer requires both seals under the exact terminal claim; persisted JSON alone cannot manufacture merge authority.
- Narrow recovery for a historically merged release candidate requires the exact immutable candidate, bundle, task, event chronology, failed-validation evidence, PR identity, merge SHA/time, and protected-target ancestry. Recovery preserves the failed validation as an explicit warning; observing an already-deployed merge never rewrites that historical failure into a pass.
